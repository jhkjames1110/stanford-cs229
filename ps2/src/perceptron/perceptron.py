...
def initial_state():
    ...
    # *** START CODE HERE ***
    # tuple of (examples, beta)
    return ([], [])
    # *** END CODE HERE ***


def signHypothesis(z):
    if (z >= 0):
        return 1
    else:
        return 0
...
def predict(state, kernel, x_i):
    ...
    for j in range(len(x)):
        prediction = prediction + beta[j] * kernel(x[j],x_i)

    return signHypothesis(prediction)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    new_beta = learning_rate * (y_i - predict(state, kernel, x_i))
